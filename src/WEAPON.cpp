...
void WEAPON::update(float Posi_x, float Posi_y, float dir_x, float dir_y,  short currentFrame)
{
   std::map<std::string, struct GunAttribute>::iterator it;
   short Direction = (dir_x > 0? 1: -1);
    ...
    this->bulletGenPosi.y = GunModel.getPosition().y + (MuzzelPoint.y * dir_y);
}
