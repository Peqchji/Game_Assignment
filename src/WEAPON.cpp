...
void WEAPON::shotingOut(std::string &Type, float dir_x, float dir_y, float &playerVelocity_x, float &playerVelocity_y, std::vector<BULLET*> &bullets)
{
  ...
  else if(gatlingShot)
  {
    ...
    bullets.push_back(new BULLET(this->bulletGenPosi.x + gatlingCounter * 3, this->bulletGenPosi.y - gatlingCounter * ((dir_x < 0 && dir_y > 0) || (dir_y < 0 && dir_x > 0)? -3:3), dir_x, dir_y, this->Dealing_Damge, 250.f));
...
void WEAPON::setGunType()
{
  GunType = 
  {
    ...
    {"Tactical Shotgun", GunAttribute(2, 3, 5, 2, sf::Vector2f(23.f, -3.f), sf::Vector2f(6.f, 8.f), "../content/Gun/Sprite/Tactical_Shotgun.png")},
    {"Gatling Gun", GunAttribute(2, 2, 0, 10, sf::Vector2f(29.f, 0.f), sf::Vector2f(8.f, 5.f), "../content/Gun/Sprite/Gatling_Gun.png")},
    {"Sniper Rifle", GunAttribute(16, 4, 45, 1.2, sf::Vector2f(36.f, -2.f), sf::Vector2f(15.f, 9.f), "../content/Gun/Sprite/Sniper_Rifle.png")}
  };
}